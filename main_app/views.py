...
def shopping_list_index(request):
    ...
    return render(
        request, "main_app/shopping_lists.html", {"shopping_lists": shopping_lists}
    )
...
def add_recipe(request):
    recipe, _ = Recipe.objects.get_or_create(
        api=request.POST.get("api"), name=request.POST.get("name")
    )
...
def search_recipe(request):
    ...
    recipes = requests.get(
        "https://api.spoonacular.com/recipes/complexSearch", parameters
    ).json()["results"]
...
def get_recipe(request, id):
    ...
    recipe = requests.get(
        f"https://api.spoonacular.com/recipes/{id}/information", parameters
    ).json()
